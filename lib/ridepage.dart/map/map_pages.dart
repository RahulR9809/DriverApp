...
class _MapboxPicupSimulationState extends State<MapboxPicupSimulation>
  ...
  void _animateSymbolAlongRoute() async {
    ...
    if (routeLine != null) {
      await mapController.updateLine(
          routeLine!, LineOptions(geometry: routeCoordinates));
    }
    ...
    for (int i = 0; i < steps; i++) {
      ...
      for (LatLng currentPosition in interpolatedPoints) {
        ...
        await mapController.updateLine(
            routeLine!, LineOptions(geometry: updatedRoute));

        await Future.delayed(Duration(
            milliseconds: (stepDuration * 1000 / interpolationSteps).toInt()));
      }
    }
  ...
  void _triggerReachedAnimation() {
    ReachedDialog.showLocationReachedDialog(context,
        text:
            'You have arrived at the pickup location. Please proceed with the pickup',
...
class _MapboxDropSimulationState extends State<MapboxDropSimulation>
  ...
  void _triggerReachedAnimation() {
    ReachedDialog.showLocationReachedDialog(context,
        ...
        text:
            'You have arrived at the drop location. Please assist the passenger with the drop-off.',
        onConfirm: () {
