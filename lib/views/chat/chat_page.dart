...
class _ChatPageState extends State<ChatPage> {
            ...
            child: BlocListener<ChatBloc, ChatState>(
              listener: (context, state) {
                if (state is ChatReceivedMessagesUpdated ||
                    state is ChatSentMessagesUpdated) {
                  _scrollToBottom();
                }
  ...
  Widget _buildChatList(List<Map<String, dynamic>> messages) {
    if (kDebugMode) {
      print(
          "ChatPage: _buildChatList called, message count = ${messages.length}");
    }
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        if (kDebugMode) {
          print(
              "ChatPage: _buildChatBubble called, message = ${messages[index]['message']}");
        }
        return _buildChatBubble(
          message: messages[index]['message'],
          isSender: messages[index]['isSender'],
          time: messages[index]['time'],
          avatar: messages[index]['isSender']
              ? 'asset/chatDriver.png'
              : 'asset/chatUser.png',
        );
      },
    );
